            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="max-w-md mx-auto">
                        <form action="{{ route('home.update',[$post->slug]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <div class="mb-4">
                                <label for="title" class="block text-gray-700 font-bold mb-2">Article Title</label>
                                <input type="text" id="title" name="title" placeholder="Article Title" value="{{ $post->title }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea id="description" name="description" value="" placeholder="Description for Your Article" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500">{{ $post->sedcription}}</textarea>
                                <div class="error text-red-500 mt-1">
                                    @error('description')
                                        {{ $message }}
                                    @enderror
